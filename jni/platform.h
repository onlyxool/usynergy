 ...
 * This function is called when uSynergy needs to connect to the host.
 * It doesn't imply a network implementation or destination address, that must
 * all be handled on the user side. The function should return USYNERGY_TRUE
 * if a connection was established or USYNERGY_FALSE if it could not connect.
 ...
 * @param outLength	Address of integer that receives the actual amount of bytes
 ...
 * uSynergy onli traces major events like connecting and disconnecting. Usually
 ...
 * This callback is called when a mouse events happens. The mouse X and
 * Y position, wheel and button state is communicated in the message. It's
 * up to the user to interpret if this is a mouse up, down, double-click or
 * other message.
 ...
 * @param down		Down or up status, 1 is key is pressed down,
 *	0 if key is released (up)
 ...
 * change as these are individual messages in the packet stream. Each
 * callback will contain all the valid state for the different axes and
 * buttons. The last callback received will represent the most current joystick
 * state.

 * @param cookie		Cookie supplied in the Synergy context
 * @param joyNum		Joystick number, always in the range
 *	[0 ... USYNERGY_NUM_JOYSTICKS>
 ...
 */
static void uSynergyJoystickCallback(uSynergyCookie cookie, uint8_t joyNum,
...
*/
static void uSynergyClipboardCallback(uSynergyCookie cookie,
